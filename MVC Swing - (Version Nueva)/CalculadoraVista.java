...
public class CalculadoraVista extends JFrame {
    ...
    public CalculadoraVista() {
        ...
        setSize(380, 720); // Ajustado para un aspecto estilizado y alto
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setLocationRelativeTo(null);
        setLayout(new BorderLayout());
        
        // Color oscuro de fondo principal
        Color colorFondo = new Color(23, 23, 28);
        getContentPane().setBackground(colorFondo);

        // 1. Crear y configurar la pantalla (NORTE)
        pantalla = new JTextField();
        pantalla.setFont(new Font("Segoe UI", Font.BOLD, 42));
        pantalla.setHorizontalAlignment(JTextField.RIGHT);
        pantalla.setEditable(false);
        pantalla.setBackground(colorFondo);
        pantalla.setForeground(Color.WHITE);
        pantalla.setCaretColor(colorFondo);
        pantalla.setBorder(new EmptyBorder(40, 25, 30, 25)); // Espaciado elegante
        add(pantalla, BorderLayout.NORTH);

        // 2. Crear el panel de botones (CENTRO)
        JPanel panelBotones = new JPanel();
        panelBotones.setLayout(new GridLayout(8, 4, 8, 8)); // 8 filas, 4 columnas, 8px de separación
        panelBotones.setBackground(colorFondo);
        panelBotones.setBorder(new EmptyBorder(10, 20, 20, 20)); // Margen externo
        
        botones = new JButton[32];
        for (int i = 0; i < 32; i++) {
            String texto = etiquetasBotones[i];
            botones[i] = new BotonPersonalizado(texto, 18);
            ...
            if (texto.matches("[/*\\-+=]")) {
                ...
                botones[i].setForeground(Color.WHITE);
            } else if (i < 16) { // Científicos (primeras 4 filas)
                botones[i].setFont(new Font("Segoe UI", Font.BOLD, 16));
                botones[i].setBackground(new Color(31, 31, 37)); // Gris oscuro
                botones[i].setForeground(new Color(0, 229, 255)); // Celeste/Cian para destacar
            } else {
                botones[i].setBackground(new Color(44, 44, 53)); // Gris medio para números
                botones[i].setForeground(Color.WHITE);
            }
